---
title: "DE Analyse EdgeR Wild type contre mutant"
...
On cherche à voir les gènes différentiellement exprimés entre les wild type et le mutant icpk en condition de nitrate à 15min.

# Les CPK

La papier "Discovery of nitrate–CPK–NLP signalling in central nutrient–growth networks" met en avant le rôle des CPK, des Calcium (rôle de signalement cellulaire) sensor Protein Kinases (molécule dont le role est de modifier chimiquement d'autres molécules cibles ajout l'ajout d'un groupe phosphate). Elles jouent un role de **master regulators dans la réponse primaire au nitrate**. En particulier, elles vont phosphoryler les facteurs de transcription NLP qui orchestrent de nombreux processus de nutrition et de développement.

Les auteurs créent des mutants CPK10, CPK30, CPK32, et NLP7 pour le prouver.
...
data = dplyr::select(data, -c("Gene", "WT.K1", "WT.K2", "WT.K3"))
genes = which(!(grepl("__", rownames(data))))
data = data[genes,]
data = data[, sort(colnames(data))]

data_mutant = read.table("quantif_mutants_unstranded.txt", h=F)
colnames(data_mutant) <- c("Gene", "icpk.N1", "icpk.N2", "icpk.N3")
rownames(data_mutant)=data_mutant$Gene
data_mutant = dplyr::select(data_mutant, -c("Gene"))
data_mutant = data_mutant[genes,]

for (var in colnames(data_mutant)){
  data[,var] = data_mutant[, var]
}
...
On a beauoup de gènes différentiellement exprimés avec un seuil pour la pvalue ajustée de 5%. 
...
save(file = "DEGenesEdgeRMutant.RData", DEGenesNames)
```

Sur ces 1800 gènes gènes DE, 172 contiennent le mot "kinase" dans leur description, 33 le mot calcium, plusieurs NLP, 10 liés aux nitrates.

```{r ontshort}
library(data.table)
kable(r[r$description %like% "kinase", ])
kable(r[r$description %like% "Calcium" | r$description %like% "calcium", ])
kable(r[r$external_gene_name %like% "NLP", ])
kable(r[r$description %like% "nitrate" | r$external_gene_name %like% "NI", ])
kable(r[r$external_gene_name %like% "HHO", ])
```
