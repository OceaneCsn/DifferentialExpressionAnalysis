...
Petit cours : https://www.youtube.com/watch?v=5tGCBW3_0IA

Les deux méthodes présentées reposent sur le fait que les gènes sont en majorité non differentiellement exprimés, et que les DE sont équitablement répartis entre up et down? On calcule un sizeFactor propre à chaque échantillon.

Ne jamais modifier les données avant de les donner à un package qui normalise.
...
La dispersion est le coefficient de variation au carré. Différent de la variance. CV le coefficient de variation : mesure de la variabilité relative (ratio écart type sur moyenne).
...
Suivant la formule des variances de mixture distributions, la variance du modèle est $var(reads_{i,j}) = \mu_{i,j}+\phi_i\mu_{i,j}^2$. Elle est composée du "shot noise" et de la variance brute. En divisant par $\mu_{i,j}^2$ on obtient $CV^2(reads_{i,j}) = \frac{1}{\mu_{i,j}}+\phi_i =  technical CV^2 +  biological CV^2$. 

Le CV technique diminue avec l'augmentation du nombre de reads, tandis que le CV biologique reste constant. C'est ce paramètre biologique $\phi_i$ qui est à estimer avant de pouvoir tester statistiquement l'expression différentielle pour un gène.


On peut utiliser l'ensemble des gènes pour estimer cette variance, étant donné la nature parallèle du RNA Seq. Cela est très utile étant donné le peu de valeurs d'expression pour un gène. Pour cela, on peu considérer une fonction de la moyenne d'expression est donc utilisée pour estimer la variance, ce qui permet de pooler tous les gènes d'expression similaire et de pallier au manque de valeurs.

C'est pour cela qu'on utilise en fait une loi quasi négative binomiale, dont la variance s'écrit $var(reads_{i,j}) = \sigma_i^2(\mu_{i,j}+\phi_i\mu_{i,j}^2)$.
...
On estime les variances par maximum de vraissemblance, conditionnellement au nombre total de reads par gène (qCML methods), et GLM.
...
Inférence bayesienne, avec un prior sur $\phi_i$ log normal centré autour d'une fonction à priori de l'expression moyenne du gène. 
Méthode bayesiennes empiriques par maximum de likelihood pour estimer $\phi_i$ à partir des données. Puis, on fitte la tendance variance en fonction de l'expression moyenne. Enfin, on shrink les estimations grace à la fonction fittée.
...
On crée un test exact (test permettant de conclure à la significativité tout en fixant l'erreur de première espèce)

la statistique de test est la somme des counts pour les deux conditions : $K_{iA} + K_{iB} = \Sigma_{j \in A} reads_{i,j} + \Sigma_{j \in B} reads_{i,j}$. A partir du modèle d'erreur, on peut calculer la probabiliter d'obtenir l'énènement $K_{iA} = a$ et $K_{iB} = b$, $p(a,b)$.
...
Les logFoldChange sont estimés estimés par méthodes bayesiennes, et sont centrés en 0.
On teste le logFoldChange estimé divisé par son écart type, donne une statistique z testée contre une distribution normale centrée réduite.
