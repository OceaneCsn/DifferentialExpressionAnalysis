...
s.corcircle(acp$co)
s.class(acp$li, factor(data$cluster), col = brewer.pal(n = 10, name = "Paired"), chullSize = 1, ellipseSize = 0, plabels.cex = 0.7, pbackground.col = "grey85", main = "Clusters dans le plan principal", ylim = c(-4, 4))
...
ggplot(logData, aes(x=(meanN+meanK)/2, y=meanN-meanK, color=as.character(cluster))) +
  geom_point(size = 0.8) + scale_color_brewer(palette="Paired") + ggtitle("Mais en fait... on retrouve un MA plot avec clustering!")
```
...
- La différence d'expression entre K et N (ordonnée de l'ACP). Les clusters 6, 8, 10, 3, 7, 1 et 0 comprtent des gènes uprégulés par le nitrate, les autres sont down régulés. Le clustering a été fait sur la force de l'expression différentielle, d'où cette apparence de "couches" parallèles pour les clusters.
...
On parvient à exhiber des processus biologiques en faisant des tests sur une loi hypergéométrique, dont l'hypothèse nulle est que tous les gènes ont la même ontologie. La distribution "bachground" est composée de tous les gènes annotés.
...
On obtient bien une clique associée au métabolisme des nitratesn, transport d'ions, et réponse à la vcarence nutritionnelle.

Les autres, je sais pas spécialement comment les interprêter.
